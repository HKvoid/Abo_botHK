...
class ColorSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if new_role: await member.add_roles(new_role); await interaction.followup.send(f"✅ Tu color ahora es {new_role.mention}", ephemeral=True)
...
def build_color_embed():
    """Construye el embed visual del menú de colores."""
    # Agrupa colores en líneas con bullet de color usando hex
    lines = []
    for name, hex_val in COLOR_ROLES.items():
        r = (hex_val >> 16) & 0xFF
        g = (hex_val >> 8) & 0xFF
        b = hex_val & 0xFF
        lines.append(f"🔘 **{name}** — `#{hex_val:06X}` (R:{r} G:{g} B:{b})")

    embed = discord.Embed(
        title="🎨 Color Roles",
        description=(
            "Elige el color de tu nombre en el servidor.\n"
            "Selecciona uno del menú de abajo. Solo puedes tener **un color activo** a la vez.\n\u200b"
        ),
        color=0x9B59B6
    )
...
class ColorsCog(commands.Cog):
    ...
    @app_commands.command(name="color", description="Abre el menú pa cambiar tu color de nombre")
    async def color_menu(self, interaction: discord.Interaction):
        embed = build_color_embed()
