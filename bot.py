...
ROLES_STAFF_COLOR = ["Root", "ViceRoot", "Admin", "Moderador"]
...
def is_staff_color():
    """Solo Root, ViceRoot, Admin y Moderador pueden usar /staffcolor."""
    async def predicate(interaction: discord.Interaction) -> bool:
        if interaction.user.id == TU_ID: return True
...
# ─────────────────────────────────────────
STAFF_COLOR_ROLES = {
    "Ignis Petal": 0xFF4E50,
    "Arctic Sky": 0x5DADE2,
    "Midnight Pulse": 0x2C3E50,
    "Peach Parfait": 0xFFB6C1,
    "Electric Soul": 0x8E44AD,
    "Phoenix Ember": 0xE67E22,
    "Cyan Sand": 0xA3E4D7,
    "Abyssal Echo": 0x1A5276,
    "Lunar Silk": 0xBCC6CC,
    "Aurora Borealis": 0x2ECC71,
}

class StaffColorSelect(discord.ui.Select):
    def __init__(self):
        options = [discord.SelectOption(label=name, emoji="🌈") for name in STAFF_COLOR_ROLES.keys()]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != TU_ID and not any(rol.name in ROLES_STAFF_COLOR for rol in interaction.user.roles):
            return await interaction.response.send_message("❌ Este menú es exclusivo para Staff (Root, ViceRoot, Admin, Moderador).", ephemeral=True)

        await interaction.response.defer(ephemeral=True)
        guild, member, color_name = interaction.guild, interaction.user, self.values[0]
        roles_a_quitar = [discord.utils.get(guild.roles, name=n) for n in STAFF_COLOR_ROLES.keys()]
        roles_a_quitar = [r for r in roles_a_quitar if r]
        ...
        if new_role:
            ...
            await interaction.followup.send(f"❌ No existe el rol `{color_name}`. Usa `/create_staffcolors` we.", ephemeral=True)
...
class StaffColorsCog(commands.Cog):
    ...
    async def staffcolor(self, interaction: discord.Interaction):
        embed = discord.Embed(
            ...
            description=(
                "Colores exclusivos para **Root, ViceRoot, Admin y Moderador**.\n"
    ...
    @app_commands.command(name="create_staffcolors", description="[Staff] Crea los roles de color exclusivos de Staff")
    @is_staff_color()
    async def create_staffcolors(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True); creados = 0
        for name, hex in STAFF_COLOR_ROLES.items():
            if not discord.utils.get(interaction.guild.roles, name=name):
                await interaction.guild.create_role(name=name, colour=discord.Colour(hex), reason="Abo Staff Colors"); creados += 1
        await interaction.followup.send(f"✅ Creados {creados} roles de Staff. Los otros ya existían.", ephemeral=True)
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        if es_staff:
            ...
            embed.add_field(name="🌈 Staff Colors 🔒", value=(
                "`/staffcolor` — Elige tu color exclusivo de Staff (solo Root, ViceRoot, Admin, Moderador)\n"
                "`/create_staffcolors` — Crea los roles de color exclusivos de Staff"
            ), inline=False)
...
@bot.event
async def on_ready():
    await bot.add_cog(ColorsCog(bot)); await bot.add_cog(StaffColorsCog(bot)); await bot.add_cog(ModCog(bot)); await bot.add_cog(PurgaCog(bot))
