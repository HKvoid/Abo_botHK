...
class ColorSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        roles_a_quitar = [r for r in (discord.utils.get(guild.roles, name=n) for n in COLOR_ROLES.keys()) if r]
        new_role = discord.utils.get(guild.roles, name=color_name)
        if not new_role:
            return await interaction.followup.send(f"❌ No existe el rol `{color_name}`. Usa `/create_colors` we.", ephemeral=True)
        try:
            ...
            await interaction.followup.send("❌ No tengo permisos para asignar ese rol we (revisa mi jerarquía).", ephemeral=True)
    ...
    async def create_colors(self, interaction: discord.Interaction):
        ...
        for name, color_hex in COLOR_ROLES.items():
            if not discord.utils.get(interaction.guild.roles, name=name):
                await interaction.guild.create_role(name=name, colour=discord.Colour(color_hex), reason="Abo Colors"); creados += 1
...
class StaffColorSelect(discord.ui.Select):
    def __init__(self):
        options = [discord.SelectOption(label=name, emoji="🌈") for name in STAFF_COLOR_ROLES]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if interaction.user.id != TU_ID and not any(rol.name in ROLES_STAFF_COLOR for rol in interaction.user.roles):
            return await interaction.response.send_message("❌ Este menú es exclusivo para el rol Staff.", ephemeral=True)

        await interaction.response.defer(ephemeral=True)
        guild, member, color_name = interaction.guild, interaction.user, self.values[0]
        roles_a_quitar = [r for r in (discord.utils.get(guild.roles, name=n) for n in STAFF_COLOR_ROLES) if r]
        new_role = discord.utils.get(guild.roles, name=color_name)
        if not new_role:
            return await interaction.followup.send(f"❌ El rol `{color_name}` no existe todavía en el servidor. Pide a un admin que lo cree.", ephemeral=True)
        try:
            if roles_a_quitar: await member.remove_roles(*roles_a_quitar, reason="Cambio de color Staff")
            await member.add_roles(new_role)
            await interaction.followup.send(f"✅ Tu color de Staff ahora es {new_role.mention}", ephemeral=True)
        except discord.Forbidden:
            await interaction.followup.send("❌ No tengo permisos para asignar ese rol we (revisa mi jerarquía).", ephemeral=True)

class StaffColorSelectView(discord.ui.View):
    def __init__(self): super().__init__(timeout=60); self.add_item(StaffColorSelect())
...
class StaffColorsCog(commands.Cog):
    ...
    async def staffcolor(self, interaction: discord.Interaction):
        ...
        for name in STAFF_COLOR_ROLES:
            rol = discord.utils.get(interaction.guild.roles, name=name)
        ...
        await interaction.response.send_message(embed=embed, view=StaffColorSelectView())
...
def is_booster():
    """Solo quienes tengan el rol 'Server Booster' (o estén boosteando) pueden usar /boostercolor."""
    async def predicate(interaction: discord.Interaction) -> bool:
        return _es_booster(interaction.user)
    return app_commands.check(predicate)
...
class BoosterColorSelect(discord.ui.Select):
    def __init__(self):
        options = [discord.SelectOption(label=name, emoji="💎") for name in BOOSTER_COLOR_ROLES.keys()]
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if not _es_booster(interaction.user):
            return await interaction.response.send_message("❌ Este menú es exclusivo para Server Boosters we.", ephemeral=True)

        await interaction.response.defer(ephemeral=True)
        guild, member, color_name = interaction.guild, interaction.user, self.values[0]
        roles_a_quitar = [r for r in (discord.utils.get(guild.roles, name=n) for n in BOOSTER_COLOR_ROLES.keys()) if r]
        new_role = discord.utils.get(guild.roles, name=color_name)
        if not new_role:
            return await interaction.followup.send(f"❌ No existe el rol `{color_name}`. Usa `/create_booster_colors` we.", ephemeral=True)
        try:
            if roles_a_quitar: await member.remove_roles(*roles_a_quitar, reason="Cambio de color Booster")
            await member.add_roles(new_role)
            await interaction.followup.send(f"✅ Tu color de Booster ahora es {new_role.mention}", ephemeral=True)
        except discord.Forbidden:
            await interaction.followup.send("❌ No tengo permisos para asignar ese rol we (revisa mi jerarquía).", ephemeral=True)

class BoosterColorSelectView(discord.ui.View):
    def __init__(self): super().__init__(timeout=60); self.add_item(BoosterColorSelect())

# ── Vista pública con solo el botón (persistente, timeout=None) ──
class BoosterColorButtonView(discord.ui.View):
    def __init__(self): super().__init__(timeout=None)

    @discord.ui.button(label="💎 Seleccionar color Booster", style=discord.ButtonStyle.primary, custom_id="booster_color_button")
    async def abrir_selector(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not _es_booster(interaction.user):
            return await interaction.response.send_message("❌ Este menú es exclusivo para Server Boosters we.", ephemeral=True)
        embed_privado = discord.Embed(
        ...
        await interaction.response.send_message(embed=embed_privado, view=BoosterColorSelectView(), ephemeral=True)

def build_booster_color_embed(guild: discord.Guild):
    """Embed público con la lista de colores de Booster disponibles y un botón."""
    lines = []
    ...
    embed = discord.Embed(
        ...
        description=(
            "Colores exclusivos para **Server Boosters**.\n"
            "Presiona el botón de abajo para elegir. Solo puedes tener **un color activo de Booster** a la vez.\n\u200b"
        ),
        color=0xF47FFF
    )
    embed.add_field(name="Colores disponibles", value="\n".join(lines), inline=False)
    embed.set_footer(text="Abo Colors • Exclusivo para quienes boostean el server")
    return embed

class BoosterColorsCog(commands.Cog):
    def __init__(self, bot): self.bot = bot

    @app_commands.command(name="boostercolor", description="[Booster] Muestra el menú de colores exclusivo de Server Booster")
    @is_booster()
    async def boostercolor_menu(self, interaction: discord.Interaction):
        embed = build_booster_color_embed(interaction.guild)
        await interaction.response.send_message(embed=embed, view=BoosterColorButtonView())
    ...
    async def create_booster_colors(self, interaction: discord.Interaction):
        ...
        for name, color_hex in BOOSTER_COLOR_ROLES.items():
            if not discord.utils.get(interaction.guild.roles, name=name):
                await interaction.guild.create_role(name=name, colour=discord.Colour(color_hex), reason="Abo Booster Colors"); creados += 1
        await interaction.followup.send(f"✅ Creados {creados} roles de Booster. Los otros ya existían.", ephemeral=True)
...
class ModCog(commands.Cog):
    ...
    async def ban(self, interaction: discord.Interaction, user: discord.Member, reason: str = "Se pasó de verga"):
        if user.id == interaction.user.id:
            return await interaction.response.send_message("¿Estás loc@?", ephemeral=True)
    ...
    async def mute(self, interaction: discord.Interaction, user: discord.Member, time: str = "10m"):
        ...
        if unidad not in mult or not time[:-1].isdigit():
            return await interaction.response.send_message("❌ Formato inválido we. Usa `10m`, `2h`, `1d`", ephemeral=True)
        tiempo_seg = int(time[:-1]) * mult[unidad]
        if tiempo_seg <= 0 or tiempo_seg > 28 * 86400:
            return await interaction.response.send_message("❌ El tiempo debe ser mayor a 0 y máximo 28 días we", ephemeral=True)
        await interaction.response.defer()
        try:
            ...
            await interaction.followup.send("❌ No tengo permisos pa mutearlo we")
        except discord.HTTPException as e:
            log.error(f"[Mute Error] {e}")
            await interaction.followup.send("❌ No se pudo mutear, intenta de nuevo we")
    ...
    async def addrol(self, interaction: discord.Interaction, rol: discord.Role, users: str):
        ...
        if interaction.user.id != TU_ID and rol.position >= interaction.user.top_role.position:
            return await interaction.response.send_message("No puedes dar un rol superior al tuyo", ephemeral=True)
        await interaction.response.defer()
        exitos, fallos = [], []
        for uid in mentions:
...
class WarnsCog(commands.Cog):
    ...
    async def unwarn(self, interaction: discord.Interaction, user: discord.Member, cantidad: app_commands.Range[int, 1, 100] = 1):
        ...
        if not ids:
            return await interaction.followup.send(f"✅ {user.mention} no tiene warns we.")
...
class HelpCog(commands.Cog):
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        embed.add_field(name="🎨 Color", value="`/color` — Elige tu color de nombre", inline=False)
        if _es_booster(interaction.user):
            embed.add_field(name="💎 Booster Color", value="`/boostercolor` — Elige tu color exclusivo de Server Booster", inline=False)
        if es_staff:
            ...
            embed.add_field(name="💎 Booster Colors 🔒", value=(
                "`/boostercolor` — Elige tu color exclusivo de Booster (solo Server Boosters)\n"
...
async def on_ready():
    ...
    if not _bot_iniciado:
        ...
        bot.add_view(ColorButtonView())  # re-registra la vista persistente tras reinicios
        bot.add_view(BoosterColorButtonView())  # re-registra la vista persistente de Booster Colors tras reinicios
        _bot_iniciado = True
...
async def on_app_command_error(interaction: discord.Interaction, error: app_commands.AppCommandError):
    if isinstance(error, app_commands.CheckFailure):
        msg = "❌ No tienes permiso para usar este comando we."
    else:
        log.error(f"[Command Error] {interaction.command}: {error}")
        msg = "❌ Algo salió mal ejecutando el comando we."
    try:
