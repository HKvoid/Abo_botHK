...
class WarnsCog(commands.Cog):
    ...
    @app_commands.command(name="warns", description="[Staff] Muestra el historial de warns de un usuario")
    ...
    async def clearwarns(self, interaction: discord.Interaction, user: discord.Member):
        ...
        db.commit()
        await interaction.followup.send(f"🧹 Warns de {user.mention} borrados.", ephemeral=True)
    ...
    async def help(self, interaction: discord.Interaction):
        ...
        if es_staff:
            embed.add_field(name="⚠️ Warns 🔒", value=(
                ...
                "`/warns <user>` — Ve el historial de warns\n"
                "`/clearwarns <user>` — Borra los warns de un usuario"
            ), inline=False)
